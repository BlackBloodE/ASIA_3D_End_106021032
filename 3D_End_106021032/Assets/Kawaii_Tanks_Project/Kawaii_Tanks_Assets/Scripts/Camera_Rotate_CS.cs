...
namespace ChobiAssets.KTP
{
	...
	public class Camera_Rotate_CS : MonoBehaviour
	{
		...
		void Mouse_Input_Drag (bool isFreeAiming)
		{
			...
			float rotationX = transform.localEulerAngles.y + Input.GetAxis("Mouse X") * sensitivityY;
			...
			if (idScript.aimButton == false) {
				...
				if (!idScript.dragButton) {
					...
					//float horizontal = (move.x - previousMousePos.x) * 0.1f;
					//targetAng_Y += rotationX;
					//angY = targetAng_Y;
					//print(angY);
			...
			angY = Mathf.MoveTowardsAngle (angY, targetAng_Y, 180.0f * Time.deltaTime);
			//thisTransform.rotation = Quaternion.Euler (0.0f, 0, angZ);
			//設定攝像機角度
			thisTransform.localEulerAngles = new Vector3(0, rotationX, 0);
			print(rotationX);
			thisTransform.rotation = Quaternion.Euler(0.0f, 0, angZ);
		}
		#endif
